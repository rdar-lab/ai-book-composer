...
class FileListingTool:
    ...
    def __init__(self, settings: Settings, input_directory: str):
        ...
        try:
            for file_path in self.input_directory.rglob("*"):
                if file_path.is_file():
                    # Security check
                    if not is_path_safe(self.settings, self.input_directory, file_path):
...
class TextFileReaderTool:
    ...
    def run(self, file_path: str, start_line: int = 1, end_line: Optional[int] = None) -> Dict[str, Any]:
        ...
        if not is_file_size_within_limits(self.settings, file_path):
        ...
        try:
            ...
            if end_line is None:
                end_idx = min(start_idx + self.settings.text_reading.max_lines_per_read, total_lines)
            else:
...
class VideoTranscriptionTool:
    ...
    def __init__(self, settings: Settings):
        """Initialize transcription tool."""
        self.settings = settings
        self.audio_tool = AudioTranscriptionTool(settings)
...
class ChapterWriterTool:
    ...
    def run(self, chapter_number: int, title: str, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Security check
            if not is_path_safe(self.settings, self.output_directory, file_path):
            ...
            logger.exception(f"Error writing chapter: {e}")
...
class ChapterListWriterTool:
    ...
    def run(self, chapters: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error writing chapter list: {e}")
            return {"success": False, "error": str(e)}
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                for img_index, img in enumerate(image_list):
                    ...
                    # Security check
                    if not is_path_safe(self.settings, self.images_directory, image_path):
                        logger.warning(f"Skipping unsafe image path: {image_path}")
                        continue

                    # Check image size
                    image_size_mb = len(image_bytes) / (1024 * 1024)
                    if image_size_mb > self.settings.image_processing.max_image_size_mb:
                        logger.warning(
                            f"Image too large: {image_size_mb}MB > {self.settings.image_processing.max_image_size_mb}MB")
                        continue
...
class ImageListingTool:
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        try:
            supported_formats = set(self.settings.image_processing.supported_formats)
